...
| Newcomer scope | Issue body, labels, and Comments: requested changes, maintainer clarifications, unresolved design discussions, and previous implementation attempts. | Pass if the issue describes one bounded implementation or fix without requiring a major redesign, unresolved architectural decision, or substantial changes to core internals. Reject umbrella or tracking issues, pure usage questions, and work explicitly identified as too complex for a newcomer. Missing reproduction steps or a short description alone do not cause failure. | required |
